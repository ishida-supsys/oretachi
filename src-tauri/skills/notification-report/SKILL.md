...
- **レポートは原則作る（#268）。** **カードが 1 枚でも残れば必ず 1 枚作る。** 作らないのは
  Step 1-2 の母集合が 0 件のとき、および 1-2 の除外（購読外 / 発信元がクローズ済み）で
  カードが 1 枚も残らなかったときだけ。「件数が少ない」「内容が軽い」「ターミナルを開けば分かる」
...
1-2 の除外（購読外 / `sourceWorktreeName` が `null`）でカードが 1 枚も残らなかったときだけ。
...
- **レポートを作るかどうかを価値判断で決めない（#268）。** カードが 1 枚でも残れば作る
  （作らないのは母集合 0 件と、1-2 の除外でカードが 1 枚も残らなかったときだけ）。
  「件数が少ない」「内容が軽い」「ターミナルを見れば分かる」は省略の理由にならない。
- **母集合の通知を「もう回答済みに見えるから」で落とさない（#268）。** カードを落としてよいのは
  購読外と `sourceWorktreeName` が `null` の 2 つだけ（→ 1-2d）。回答済みかは通知からは分からず、
  同じワークツリーの 2 件目以降もカードとしては全部載せる（キー操作を 1 枚に絞るのとは別の話。→ 1-4e）。
- **母集合が 0 件のとき、および 1-2 の除外でカードが 1 枚も残らなかったとき（#268）も黙って終わらない。** 1-2b の手順（`include_acked: true` で引き直し、3 分岐に振り分けて前回レポートの導線を返す）を必ず踏む。ここを飛ばすと「1 回出したあと二度目を出さない」に見える（#220）。
...
- **生成後の端末出力を長く書かない（#268）。** 1 行目は「通知レポート <ID> を作成しました（要返答 N 件 / 報告 M 件）」で固定し、以降は最大 2 行。カードの中身（設問文・選択肢・承認対象のコマンド）も踏んだ手順も端末へ写さない。作られたのかどうかが一目で分からなくなる。**レポートを作らなかったとき（1-2b の 3 分岐のうち前回レポートが使えない 2 つ）はこの制約の対象外**で、なぜ出せないかを件数つきで伝える（→ Step 6「生成後に端末へ出す文面」）。
