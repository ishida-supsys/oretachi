...
### 1-2b. 0 件だったとき（2 回目以降の呼び出しはここに来る）

**0 件は異常ではなく、前回のレポートで拾ったぶんを Step 5.5 で ack し切った結果。** 同じセッションで
2 回目以降にこのスキルを呼ぶと、新着が無ければ必ずここに来る。**「通知はありません」で黙って
終わらせず、必ず次の 2 つを踏むこと。** ここを飛ばすと「1 回出したあと二度目を出さない」という
形でユーザーから見える（#220）。

1. **ack 済みも含めて引き直し、何が起きているのかを確定させる。**
   ```
   oretachi_poll_inbox(terminal_id: <自分の terminal_id>, include_acked: true)
   ```
   - 何も返らない → 購読先から通知が 1 件も来ていない。1-1 の購読対象と
     `oretachi_list_worktree_notifications` を見せて「購読しているワークツリーからの通知がまだ無い」
     ことをユーザーへ伝える（レポートは作らない）。
   - `ackedAt` が入ったものだけが返る → **既に前回レポートへ載せたぶん。** それが唯一の返答窓口として
     生きているので、`search_artifact(query: "通知レポート", ...)` でそのレポートを探し、ID と導線を
     ユーザーへ提示する。**勝手に作り直さない** — 同じ通知を二重にカード化することになる。

2. **ユーザーが「もう一度出して」と言っている場合だけ作り直す。** 前回レポートが session_id 失効や
   `stale` で使えない／閉じてしまって見つからない、というのが実際の要求なので、
   「レポートを作り直すとき」の手順（`include_acked: true` で拾い直す）へ進む。**このとき ID は
   前回と必ず別にする**（→ Step 5 の ID の作り方）。

**`include_acked: true` の結果を確認せずにそのまま全件カード化してはいけない。** 既に返答済みの通知が
もう一度カードになり、ユーザーが同じ返答を宛先へ二重送信しうる。
...
- 構造が返ってきた（既にある）→ **末尾に `-2`, `-3` … を足して空いている ID を探す。** 作り直しの
  ときは母集合が前回と同じ `createdAt` を含むのでここに必ず来る
...
**ID は前回レポートと必ず別にする。** 母集合が前回と同じ `createdAt` を含むので、Step 5 の
ID の作り方どおりに `outline` で存在確認して `-2`, `-3` … を足す。同じ ID で `create` すると、
前回レポートを開いたままなら `locked_while_open` で弾かれ、閉じていれば**前回レポートを黙って
上書きする**（未返答のカードが消える）。
...
- **`oretachi_poll_inbox` が 0 件でも黙って終わらない。** 1-2b の手順（`include_acked: true` で引き直し、前回レポートの導線を返す）を必ず踏む。ここを飛ばすと「1 回出したあと二度目を出さない」に見える（#220）。
...
- **`include_acked: true` の結果をそのまま全件カード化しない。** 返答済みの通知が再びカードになり、同じ返答を宛先へ二重送信しうる。
