...
function blockedReason(n, conflicts) {
  ...
  // **補完のポップアップが開いている入力欄へは送らない（#292）。**
  //
  // `/` コマンド補完と `@` ファイル補完が開いている画面は、以前は `unknown` だったので
  // ここで機械的に塞がれていた。入力欄を読めるようにした（→ `pendingInput` で
  // 打ちかけを警告できる）副作用で、**送信可能な画面に変わっている。**
  //
  // 残っているのが `/co` / `@src/` という「**CR の意味を変える接頭辞**」であることが、
  // ただの打ちかけと質的に違う。本文を連結して CR を送ると:
  //
  // - 連結でマッチが外れてポップアップが閉じ、`/co進捗どう？` という壊れた本文が送られる
  // - 連結後もマッチが残ると、**CR が補完の確定として食われて何も送られない**
  //
  // どちらも `pendingInput` は fingerprint に入らないので `stale` では弾けない。
  // 読めるようになった価値（人が理由を読める）は保ったまま、送信だけ `unknown` 時代へ戻す。
  //
  // **`/` と `@` で発火条件が違う（#292。4〜5 周目のセルフレビュー）。**
  //
  // - `@` ファイル補完は行頭限定ではなく**行中でも発火する**（`これを見て @src/ma`）。
  //   見るのは末尾のトークン＝カーソル位置
  // - `/` コマンド補完は**入力欄の先頭でしか発火しない**。空白の後ろの `/` まで拾うと
  //   `cat /etc/hosts` のような普通の下書きで送信が塞がる
  const draft = (hasPrompt(n) && n.prompt.pendingInput) || '';
  const popupOpen = /^\/\S*$/.test(draft) || /(^|\s)@\S*$/.test(draft);
  if (shapeOf(n) === 'text' && popupOpen) {
    return (
      `'${n.worktreeName}' の入力欄に補完の打ちかけ（${draft}）が残っており、` +
      '補完のポップアップが開いています。本文を送ると打ちかけの後ろへ連結されるうえ、' +
      '末尾の Enter が補完の確定として食われて何も送られないことがあるため、' +
      'このカードからは送信できません。ターミナルを開いて打ちかけを片付けてください'
    );
  }
  if (shapeOf(n) === 'pager') {
    return (
