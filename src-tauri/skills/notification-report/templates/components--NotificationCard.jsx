...
const ACCENT = { pending: '#f9e2af', sent: '#a6e3a1', failed: '#f38ba8', pastedOnly: '#fab387' };
const MARK = { pending: '●', sent: '✓', failed: '⚠', pastedOnly: '⏸' };
const STATUS_LABEL = {
  pending: '未返答',
  sent: '返答済み',
  failed: '送信失敗',
 ...
 *   canSend  いま送れる状態か（`lib/send` の canSend の結果。再送ボタンの活性に使う）
 *   inflight この 1 件を送信中か
 *   busy     一括送信の実行中か（入力を止める）
 *   onPick / onNote / onRetry
 */
function NotificationCard({ n, answer, draft, blocked, canSend, inflight, busy, onPick, onNote, onRetry }) {
  const d = draft || {};
  ...
  const resumeEnter = status === 'pastedOnly';
  const readOnly = status === 'sent' || resumeEnter;
  ...
  return (
    <div style={{
      ...
      {readOnly ? (
        /* 返答済み / Enter 未送信: 送信内容だけを残す（候補・入力欄は出さない） */
