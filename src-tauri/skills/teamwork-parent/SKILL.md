...
- **承認待ち(`PermissionRequest`由来の`approval` = ツール許可 / プラン承認 / AskUserQuestion)は通る**(#225)。オフで止まるのは`Stop`→`completed`や高頻度な`hook`のノイズだけ。ダイアログで止まったまま気付かれない状態にはならない。
- **ユーザー判断が必要なときは`notify_worktree`を明示的に呼ぶ。トレイ通知がオフでもこれは常に通る**(このツール経由の通知は`kind`にかかわらず常にトレイへ出る)。`approval`が通るようになっても`notify_worktree`は省略しない —— ダイアログを出さずにテキストだけで問いを立てる場合、フック由来の通知は何も出ない。
