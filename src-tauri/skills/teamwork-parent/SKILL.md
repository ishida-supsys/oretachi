...
4. 5章の手順で計画フロー図をartifactとして作成する(状態はすべて`not_started`、停止条件は確定した内容を`checked: false`で入れる)。**この計画フローartifactが以後の唯一の進捗管理データになる**(別途マークダウン等は作らない)。
...
6. **承認が得られたら、Step 2 に入る直前に自分自身のワークツリーのトレイ通知をオフにする**(この呼び出し自体に承認は要らない。自分のノイズを止めるだけで他ワークツリーの通知には影響しないため):
   ...
   ```
   ここから先は子ワークツリーの購読で待機と再開を繰り返すため、自分のフック由来通知(`Stop`→`completed` 等)がトレイのノイズになる。**逆に計画が確定するまでの項番3のヒアリング・項番5の承認要求ではオンのままにしておく**(通常のフック由来通知でユーザーが呼び戻される状態を保つため、ここでは先にオフにしない)。
...
1. **そのタスクの停止条件をsub-issue本文に書き込む。** `data/flow`のタスク側`stopConditions`を、sub-issue本文の末尾に`## 停止条件`セクションとしてチェックリストで追記する(既にあれば内容を同期する):
   ...
   - [ ] 追加テーブルのスキーマが確定したら確認する <!-- sc-142-1 -->
   - [ ] 動作確認の結果をユーザーが見て判断する <!-- sc-142-2 -->
   ```
   **`data/flow`の`id`をHTMLコメントで埋め込む。** 子はこのidをクリア報告に含めるため、
   親は文面のゆらぎに関係なく対象の停止条件を特定できる。
...
- **`worktree.message`**: `oretachi_poll_inbox`で内容を確認し、`oretachi_ack_message`で既読化する。
  ...
  - **子から停止条件のクリア報告を受けたら、`data/flow`の該当条件を`checked: true`にして`checkedAt`を入れる。** 子は`停止条件クリア: [<id>]「<テキスト>」→ <ユーザーの判断内容>`の書式で送ってくる(teamwork-child 義務2)。`<id>`で対象を特定し、無ければテキストで照合する。

- **ユーザーの判断・操作が必要になったときは必ず`notify_worktree`を呼ぶ。** Step 1 の末尾(承認取得後)でトレイ通知をオフにしているため、この時点ではテキストを出力するだけではユーザーは気付けない。トレイ通知オフでも通るのは`notify_worktree`による明示通知と承認待ち(`approval`)の2経路だけで、**「ユーザーに問いを立てる」意図を確実に伝えられるのは`notify_worktree`のほう**なので、省略しない。
- すべてのイベントを5章の手順で`data/flow`の`MESSAGES`に追記する(新しい順に先頭へ追加)。
- 状態に迷ったら`oretachi_list_subscriptions`で購読状態を確認してよい。

### 親自身の停止条件(エッジ)への対応

`from`が`done`になり、次タスク`to`を起動しようとする時点で、`to`へ入るエッジ(`blocks` / `informs`いずれも)に未クリアの停止条件があれば:
...
チームワークセッションは子ワークツリーのイベントを購読して待機と再開を繰り返すため、自分自身のフック由来通知(`Stop`→`completed`など)がトレイに溢れやすい。**原則として親ワークツリー(このセッション)はトレイ通知をオフにし、人の判断・操作が必要なときにのみ明示的に`notify_worktree`で通知する。** ただしオフにするのは **Step 1 のユーザー承認を得た直後(Step 2 に入る直前)** の1回だけ(Step 1 項番6)。計画確定までのヒアリング・承認要求の区間はユーザーを呼び戻す必要が最も高いので、通常のフック由来通知が出るオンのまま残す。
...
- **承認待ち(`PermissionRequest`由来の`approval` = ツール許可 / プラン承認 / AskUserQuestion)は抑制されない**(#225)。オフで止まるのは`Stop`→`completed`や高頻度な`hook`のノイズだけなので、ダイアログで止まったまま気付かれない状態にはならない(ただしリポジトリに通知フックが1件も設定されていない場合は、この経路自体が発火しない)。
- **ユーザー判断が必要なときは`notify_worktree`を明示的に呼ぶ。トレイ通知がオフでもこれは常に通る**(このツール経由の通知は`kind`にかかわらず、リポジトリの通知フック設定にも左右されず常にトレイへ出る)。`approval`が抑制されなくなっても`notify_worktree`は省略しない —— **ダイアログを出さずにテキストだけで問いを立てる場合、フック由来の通知は何も出ない**ため。
  - 逆に`AskUserQuestion`などダイアログを伴う問いでは、`notify_worktree`と`approval`で通知が2件重なる(debounceのキーが`(ワークツリー, kind, tray)`単位なので畳まれない)。呼び戻しの確実さを優先して重複は許容する方針だが、ダイアログだけで用が足りる場面まで機械的に`notify_worktree`を足す必要はない。
