...
- Compound commands like "cd <path> && git ..." where the cd target resolves to the Current Working Directory itself are safe — cd to CWD is a no-op, so there is no bare repository risk.
