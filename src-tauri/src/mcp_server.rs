...
impl NotifyService {
    ...
    #[tool(description = "指定 PTY セッションにいま出ている「問い」を解析して返す（読み取りのみ）。返り値の JSON: { shape, navigation, header, context, questions, tabs, escapeHatch, truncated, fingerprint, tail, pendingInput, inputSuggestion, cursor, lostBytes, rows, cols, detectedAtMs }。tabs は複数設問 AskUserQuestion のタブバー（[{ label, answered, isSubmit }]。☐ が未回答 / ☒ が回答済み）で、**画面には常に 1 問ぶんしか出ない**ため questions には現在のタブの設問しか入らない。shape は \"text\"(自由入力) / \"permission\"(ツール許可ダイアログ) / \"plan\"(プラン承認) / \"askUserQuestion\" / \"yesno\"((y/N) プロンプト) / \"numbered\"(素の番号選択) / \"pager\"(less / git log などのページャ) / \"unknown\"(分類不能)。**oretachi_read_terminal のテキストからダイアログを読もうとしないこと** — Claude Code はカーソル移動で差分描画するので ANSI を除去すると断片しか残らない。このツールは出力履歴を VT エミュレータへ流し直した画面グリッドを見る。**questions[].options[].label は画面に実在する選択肢そのもの**なので、人へ提示する候補を創作せずこれを出すこと。fingerprint は oretachi_answer_prompt へそのまま渡す（画面が変わっていたら送信されない）。shape が \"unknown\" / \"pager\" のときはキーを送れない。\"unknown\" は tail を人に見せて手動操作へ誘導し、\"pager\" は何の画面かが分かっているので tail を出さずに「ターミナルで閉じて」と案内する。**入力欄の中身は pendingInput / inputSuggestion で見分けること**: Claude Code は入力待ちの入力欄に自動候補(ゴーストテキスト)を描き、画面上は人が打ったテキストと同じに見える。pendingInput が人の打ちかけ(空なら未入力)、inputSuggestion が自動候補で**ユーザー入力ではない**。tail の候補行には印が付く。どちらも fingerprint には入らない。**この 2 つと tail の印が意味を持つのは shape が \"text\" のときだけ**で、ダイアログが開いている画面(permission / plan / askUserQuestion / yesno / pager / unknown)では常に空になる —— 入力欄が塞がれているだけで**下書きが無いことの保証ではない**ので、escapeThenText で ESC 経由の本文を送るときは打ちかけの有無を確かめられない点に注意する", annotations(read_only_hint = true))]
