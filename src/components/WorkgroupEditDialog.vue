...
<i18n lang="json">
{
  "en": {
    ...
    "trayNotificationHint": "Initial value applied when a worktree is created in this group. Changing it never affects existing worktrees — switch those from each worktree's own menu. Approval waits and explicit MCP notify_worktree calls always reach the tray even when off.",
  ...
  "ja": {
    ...
    "trayNotificationHint": "このグループで新規ワークツリーを作成したときの初期値です。変更しても既存のワークツリーには影響しません（既存はワークツリー個別のメニューから切り替えてください）。オフにしても承認待ち（ツール許可・プラン承認・AskUserQuestion）と MCP notify_worktree による明示的な通知は常にトレイへ出ます。",
