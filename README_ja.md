...
- **内蔵 MCP サーバー** — Streamable HTTP による MCP サーバーで、AI エージェントが oretachi を直接操作可能。ターミナル制御（開発サーバや watcher の起動 / 出力読み取り / 入力 / 停止）、タスク・ワークツリー管理（タスク追加、ワークツリーのクローズ、ステータス・リポジトリ・アプリ設定の取得）、通知送信、アーティファクト生成に対応
- **アーティファクト** — AI エージェントが React / Markdown / HTML / SVG / Mermaid / CSV / TSV アーティファクト（ドメインモデル図・ワイヤーフレーム・スライド資料・ソート可能なデータテーブル等）を生成・編集し、専用ビューワーウィンドウでライブプレビュー
- **Claude Code プラグイン & スキル** — ワークツリーごとに Claude Code 用プラグイン（`.mcp.json`・hooks・skills）を自動生成し、`domain-model-diagram` / `wireframe` / `presentation` / `background-command` のカスタムスキルを同梱
...
- [セットアップスクリプト & 通知設定ガイド](docs/setup-guide_ja.md)
