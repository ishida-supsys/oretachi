...
- **アーティファクト** — AI エージェントが React / Markdown / HTML / SVG / Mermaid アーティファクト（ドメインモデル図・ワイヤーフレーム等）を生成・編集し、専用ビューワーウィンドウでライブプレビュー
- **Claude Code プラグイン & スキル** — ワークツリーごとに Claude Code 用プラグイン（`.mcp.json`・hooks・skills）を自動生成し、`domain-model-diagram` / `wireframe` / `background-command` のカスタムスキルを同梱
